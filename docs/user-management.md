...
Users are "identified" individuals that have a unique `id` and `email` address. Right now the email address is used instead of a "user name" when you login. It is not yet a verified email address. The supported credentials are email (as username) and password. Additionally there is a `name` field that is NOT unique and should be used by the applications for human readable presentation only.
Passwords can be changed by the users themselves. Since we do not verify email addresses and integration with a email delivery system that is not immediately marked as spam is hard, we do not have a automatic password reset flow. Instead, users with lost passwords can be helped by administrators to get a new password or a password reset link.
...
The special `admin` role gives permissions for either `tenant` management or, if present on the `global tenant *`, even the global managment. It also allows access to the full dataplane (`collections` and `channels`) of a tenant. See the sections below for details about what it enables.
...
These users can, through the `amber-client` library, or using the embedded UI, do the following workflows:

* Create and delete tenants
* Delete users
* Change passwords of users / create password reset links to share with the users
...
> to use the `amber-client` library to do that you need to add `.withAdminRole()` to the creation of the amber client. Otherwise the session will not include admin role.
...
* If and only if the users only tenant is the one being managed, the password can be changed or a password reset link can be generated by the admin
* See tenant specific monitoring data
* Edit the tenant settings and tenant name
...
> note that the special role `admin` always has full access to everything in a tenant. Even when a user has this role, it needs to be enabled in the amber client setup by using `.withAdminRole()`.
