...
The collections are defined in the startup code of your app as a chained call to your amber initialization (see [config](config.md)).
...
// client side code
 ...
 (newOrUpdated)=>{ // handle new or updated records
    const existing = myData.find((e) => e.id === newOrUpdated.id);
...
try{
    await myDataCollection.updateDoc(
        theOneToChange.id,
...
app.post('/:tenant/data/:id',async (req, res) => {
    ...
    {
        await amberApp.collections.getCollection<MyEntity>("my-data")!.deleteDocument(req.params.tenant, session.userId, req.params.id);
    }
    //..
})
```

For all methods to work with the collections from the server side, please have a look at [AmberCollection.md](api/backend/interfaces/AmberCollection.md);
...
```ts
 ...
 let highestChangeNumber = Math.max(...myData.map(e=>e.changeNumber));
