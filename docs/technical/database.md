...
* `tags` string of space separated tags, Fulltext index, for efficient addressing of documents. Fulltext index needs tags to be at least 3 characters long.
* `access_tags` string of space separated tags, Fulltext index, for access management of documents. Fulltext index needs tags to be at least 3 characters long.
...
* `change_number` int, change counter stamp
* `change_time` datetime, last update time
* `access_tags` string of space separated access tags that guarded access to the old version of the document
* `new_access_tags` string of space separated access tags that guard access to the new version of the document.
