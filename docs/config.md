...
var adminId = await amberApp.addAdminIfNotExists('admin',"Admin Account","password"); 
await amberApp.addOrUpdateTenant("default", "Default Tenant", {}); // bootstrap a default tenant.
await amberApp.auth.addRolesToUser(adminId, "default", ["editor","reader"]); // add the admin to it 
...
On the client, the library to use is `amber-client` and can be installed via `npm install amber-client`.
...
var client = amberClient().withPath("/amber").withTenant("default").withAmberUiLogin().start();
...
var user = await client.userInTenant();
console.log(`The user ${user.user.name} is logged in and is ${user.roles.includes("editor") ? "an editor" : "no editor"}`);
```
...
var user = await client.userInTenant();
client.getCollectionsApi().getCollection<BookEntity>("books").subscribe(0,
(doc)=>{
  console.log(`Received a book with title ${doc.data.title} and id ${doc.id}`);
},
(deletedDocId)=>{
  console.log(`Forget about the book with id ${deletedDocId}, it is no more`);
});
```
