...
You can find all the options for the ui (`title`, `availableRoles`, `theme`, `loginTargetUrl`, ...) in the `withUi` call above.

Once you started your backend application, you can now navigate to `/amber/ui/globaladmin` and login as an `admin` user for the `global tenant` (see [user management](user-management.md)) to manage your Amberbase instance.
...
| `/amber/ui/resetpassword?token={token}` | Set a new password using a password reset token| Everyone with a valid reset token
...
When you are building your application and want to navigate users to those views of the embedded ui, you can use the `amber-client` library to do so easily.
