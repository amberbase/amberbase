[**amberbase**](README.md)

***

# amberbase

## Classes

- [Amber](classes/Amber.md)
- [AmberInit](classes/AmberInit.md)

## Interfaces

- [AmberAuth](interfaces/AmberAuth.md)
- [AmberChannels](interfaces/AmberChannels.md)
- [AmberCollection](interfaces/AmberCollection.md)
- [AmberCollections](interfaces/AmberCollections.md)
- [ChannelSettings](interfaces/ChannelSettings.md)
- [CollectionSettings](interfaces/CollectionSettings.md)
- [ConfigOptions](interfaces/ConfigOptions.md)
- [SessionToken](interfaces/SessionToken.md)
- [SimpleWebsocket](interfaces/SimpleWebsocket.md)
- [UserContext](interfaces/UserContext.md)

## Type Aliases

- [ChannelAccessAction](type-aliases/ChannelAccessAction.md)
- [CollectionAccessAction](type-aliases/CollectionAccessAction.md)
- [WebsocketHandler](type-aliases/WebsocketHandler.md)

## Variables

- [allTenantsId](variables/allTenantsId.md)
- [sessionHeader](variables/sessionHeader.md)
- [tenantAdminRole](variables/tenantAdminRole.md)

## Functions

- [amber](functions/amber.md)
