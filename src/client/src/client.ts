...
export class AmberClientInit {
   ...
   */
  onRolesChanged(
    callback: (tenant: string | null, roles: string[], user: UserDetails | null) => void,
  ): AmberClientInit {
...
export class AmberClient {
  ...
  getCollectionsApi(): AmberCollections {
    ...
    if (this.connectionsClient == null) {
      this.connectionsClient = new AmberConnectionsClient(this.apiPrefix, tenant, () =>
  ...
  getChannelsApi(): AmberChannels {
    ...
    if (this.connectionsClient == null) {
      // collections and channels are sharing the same connection
