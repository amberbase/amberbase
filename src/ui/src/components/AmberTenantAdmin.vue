...
<template>
  <v-container>
    <v-row>
      <v-tabs v-model="tab" bg-color="amber">
        <v-tab value="users">Users</v-tab>
        <v-tab value="settings">Settings</v-tab>
        <v-tab value="data" v-if="props.tenant != '*'">Collections</v-tab>
        <v-tab value="channels" v-if="props.tenant != '*'">Channels</v-tab>
      </v-tabs>
    </v-row>
  </v-container>
  <v-container>
    <v-tabs-window v-model="tab">
            ...
            <v-card-actions>
              <v-container>
                <v-row>
                  ...
                  ></v-select>
                  <v-btn
                    @click="createInvitation(newUserRoles)"
                    :disabled="newUserRoles.length < 1"
                ...
                <v-row v-if="!!createdInvitationLink">
                  <pre style="font-size: 10pt">{{ createdInvitationLink }} </pre>
                  <v-btn
                    density="compact"
                    icon="mdi-content-copy"
                    title="copy"
                    @click="copy(createdInvitationLink)"
                  ></v-btn>
                  (valid for 14 days)
                </v-row>
              </v-container>
                    ...
                    <v-chip v-for="role in user.roles" :key="role"
                      ...
                      <template #close>
                        <v-icon
                          ...
                          @click.stop="onRemoveRole(user, role)"
                        />
                      </template>
                    </v-chip>
                  </td>
      ...
      <v-tabs-window-item value="channels">
        <amber-channels-admin :amber-client="props.amberClient" :tenant="props.tenant" />
      </v-tabs-window-item>
    </v-tabs-window>
  </v-container>
  ...
  <v-dialog v-model="pwDialogOpen" max-width="600">
    <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            @click="
              ...
              doChangePassword();
            "
            >Copy and Apply New Password</v-btn
          >
        </v-card-actions>
      </v-card-text>
      <v-card-text>
      ...
      <v-card-actions>
        ...
        <v-btn
          @click="
            copy(createdPasswordResetLink);
            pwDialogOpen = false;
          "
          >Copy Link and Close</v-btn
        >
