...
const deleteEditEntity = async () => {
  if (!editEntity.value) return;
  var confirmed = await uiHelper.confirmDialog(
    `Are you sure you want to delete document ${editEntity.value?.id}? This action cannot be undone.`,
  );
  if (!confirmed) return;
...
<template>
  <v-container>
        ...
        <v-card prepend-icon="mdi-database">
          ...
          <template v-slot:append>
            <v-btn
              ...
              density="compact"
              title="View and Sync Data into the Browser"
              @click="switchSubscription(true)"
            ></v-btn>
            <v-btn
              :disabled="!live"
              icon="mdi-sync-off"
              density="compact"
              title="Stop Data Sync"
              @click="switchSubscription(false)"
            ></v-btn>
          </template>
          <v-card-subtitle>
        ...
        <v-card class="fill-height" prepend-icon="mdi-magnify">
          <template v-slot:title>
            <v-badge
              ...
              location="right"
              color="primary"
              :content="visibleEntities.length"
            >
              Search
            </v-badge>
          ...
          <v-card-text>
            <div>
              ...
              >
                <span
                  >{{ renderSearchScope(condition.scope) }}
                ...
                <div>
                  ...
                  >
                    ...
                    <v-btn
                      ...
                      title="search based on the user who changed the document"
                    ></v-btn>
                    <v-btn
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              class="mt-2"
              color="primary"
              @click="replaceSearchConditions()"
              v-if="searchConditions.length != 1 || editSelectedConditionIndex == null"
              >New Search</v-btn
            >
          </v-card-actions>
        </v-card>
    ...
    <v-row>
      <div id="entity-list">
        <v-btn
          v-if="!dataLoaded() && !live"
          color="primary"
          prepend-icon="mdi-database-sync"
          @click="switchSubscription(true)"
          >Load data by starting synchronization</v-btn
        >
        <div
          class="entity-row"
          v-for="doc of visibleEntities"
          :key="doc.id"
          @click="selectViewEntity(doc)"
          style="cursor: pointer"
          :class="{
            selected: editEntity?.id == doc.id && showEditEntity,
            viewed: doc.id == viewEntityId,
          }"
        >
          <span class="id" :title="doc.id"> #{{ doc.id }} </span>
          <span class="time">
            <RelativeTime :date="doc.change_time" precision="minute"></RelativeTime>
          </span>
          <span class="user"> by {{ getUser(doc.change_user) }} </span>
          <span class="edit-entity-action">
            <v-btn
              icon="mdi-pencil"
              density="compact"
              title="Edit Document"
              @click.stop="selectEditEntity(doc)"
              size="small"
            ></v-btn>
          </span>
          <div
            @click.stop="selectEditEntity(doc)"
            id="selected-json"
            v-if="doc.id == viewEntityId || visibleEntities.length == 1"
          >
  ...
  <v-dialog v-model="showEditEntity">
    <v-card :loading="entityInfoBusy">
      ...
      <v-card-subtitle v-if="editEntity != null">
        ...
        <div v-if="hasTags">
          Document Tags:
          <v-chip v-for="tag in selectedEntityTags" :key="tag" outlined>{{ tag }}</v-chip>
        </div>
      </v-card-subtitle>
      <v-card-text>
      ...
      <v-card-item>
        <template v-if="hasTags">
          ...
          <div>
            ...
            <v-chip v-for="tag in editEntityTags" :key="tag" outlined>{{ tag }}</v-chip>
          </div>
        </template>
        <template v-if="hasAccessTags">
          ...
          <div>
            ...
            <v-chip v-for="tag in editEntityAccessTags" :key="tag" outlined>{{ tag }}</v-chip>
          </div>
        </template>
      </v-card-item>
      <v-card-actions>
