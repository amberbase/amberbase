  ...
  <v-app :theme="theme != 'light' ? 'amberDarkTheme' : 'amberLightTheme'">
    ...
    <v-main class="d-flex" style="min-height: 300px">
      <v-container v-if="state.uiContext.view == 'login'">
        <v-row>
          <AmberLogin
            @user-in-tenant="onUserLoggedInForApp"
      ...
      <v-container v-if="state.uiContext.view == 'reset-password'">
        <v-row>
          <AmberResetPassword
            :token="state.uiContext.passwordResetToken || ''"
            :user-email="state.uiContext.userEmail || ''"
            @password-changed="state.uiContext.view = 'login'"
          ></AmberResetPassword>
        </v-row>
      </v-container>
      ...
      <v-container v-if="state.uiContext.view == 'global-admin'">
        <v-row
          v-if="
            !amberUserInTenant ||
            amberUserInTenant.tenant != globalTenant ||
            !amberUserInTenant.roles.includes(adminRole) ||
            !amberClient
          "
        >
          <AmberLogin
            @user-in-tenant="onUserInTenant"
            :tenant="globalTenant"
            @user-ready="onUserReady"
            message="Login as global admin"
            :include-admin-role="true"
          ></AmberLogin>
        </v-row>
        <v-row v-else>
      ...
      <v-container v-if="state.uiContext.view == 'tenant-admin'">
        ...
        >
          <AmberLogin
            @user-in-tenant="onUserInTenant"
            :tenant="tenant"
            :allowGlobalTenantSelection="true"
            @user-ready="onUserReady"
            message="Login as tenant admin"
            :include-admin-role="true"
          ></AmberLogin>
        </v-row>
        <v-row v-else>
          <AmberTenantAdmin
            ...
            :roles="[adminRole, ...state.uiConfig.availableRoles]"
            @tenant-changed="
              (e) => {
                tenantName = e;
              }
