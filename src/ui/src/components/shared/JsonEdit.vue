...
var commitIfValid = async () => {
  ...
  if (buttonactionmode.value) return; // the button decides what to do
  if (validjson()) {
...
<template>
  ...
  <div class="json-edit" v-if="inEditMode">
    ...
    <span class="edit-buttons">
      ...
      <v-btn
        icon
        :disabled="!validjson()"
        @mousedown="buttonactionmode = true"
        @mouseup="buttonactionmode = false"
        @click="
          inEditMode = false;
          model = modelValue;
        "
        density="compact"
        ><v-icon title="OK">mdi-check</v-icon></v-btn
      >
      <v-btn
        icon
        @click="
        ...
        @mousedown="buttonactionmode = true"
        @mouseup="buttonactionmode = false"
        ><v-icon title="Reset">mdi-arrow-u-left-top</v-icon></v-btn
      >
      <v-btn
        icon
        @click="
  ...
  <div class="json-view" v-else>
    ...
    >
    <pre
      style="white-space: pre-wrap; word-break: break-all; margin-left: 5px; min-height: 32px"
      @click="
        inEditMode = true;
        focusTextArea();
      "
