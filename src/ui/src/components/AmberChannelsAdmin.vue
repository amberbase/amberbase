...
const channelIsPinned = (q: QualifiedChannel) =>
  channels.value.find(
    (c) =>
      (c.subscriptionScope.main == q.main && !q.sub) ||
      (c.subchannels && c.subchannels.find((s) => s.subscriptionScope.sub == q.sub)),
  );
...
const isSubscribed = (c: ChannelItem) => {
  return (
    subscribedChannelNames.value.has(c.subscriptionScope.main) ||
    subscribedChannelNames.value.has(subscriptionTrackingKey(c.subscriptionScope))
  );
};
...
function subscribe(c: ChannelItem) {
  ...
  if (subscribedChannelNames.value.has(key)) return;
...
async function sendMessage() {
  ...
  var channel = channelsClient.getChannel<any>(
    newMessageChannel.value?.subscriptionScope.main,
...
onMounted(async () => {
  ...
  channels.value = channelInfos.map((c) => ({
    name: c.name,
...
async function validateNewMessage() {
  ...
  try {
    let result = await channelsAdminApi.checkMessage(
      newMessageChannel.value.subscriptionScope.main,
      newMessageText.value,
      newMessageChannel.value.subscriptionScope.sub,
      currentUser()?.id,
  ...
  if (selectedChannel.value == null) {
    ...
    if (selectedChannel.value?.subscriptionScope.sub) {
      ...
      return messages.value
        .filter(
          (m) =>
            m.channelName.main == selectedChannel.value?.subscriptionScope.main &&
            m.channelName.sub == selectedChannel.value.subscriptionScope.sub,
        )
        .slice(0, 50);
    } else {
      // maybe the main parent is selected. Show all subchannel messages
      return messages.value
        .filter((m) => m.channelName.main == selectedChannel.value?.subscriptionScope.main)
        .slice(0, 50);
    }
  }
});
...
<template>
        ...
        <div id="channel-list">
          <template v-for="channel in channels" :key="channel.name">
            <div class="channel-item">
              <div
                :class="{ 'channel-name': true, 'selected-channel': selectedChannel == channel }"
              ...
              >
                ...
                <span class="channel-actions">
                  ...
                  <v-btn
                    v-if="isDirectSubscribed(channel)"
                    icon="mdi-sync-off"
                    title="Unsubscribe"
                    @click.stop="
                      () => {
                  ...
                  <v-btn
                    v-if="!channel.subchannels"
                    icon="mdi-message-plus-outline"
                    title="New message"
                    @click.stop="openNewMessageDialog(channel)"
                    density="compact"
                    variant="text"
                  ></v-btn>
                </span>
            ...
            <div class="subchannel-list" v-if="channel.subchannels != null">
              <div class="new-subchannel" v-if="selectedChannel == channel">
                <v-text-field
                  label="Pin Subchannel"
                  v-model="newSubchannelName"
                  @keydown.enter.prevent="addSubchannel(channel)"
                >
                  <template v-slot:append>
                    <v-btn
                      :disabled="newSubchannelName == ''"
                      icon="mdi-pin"
                      title="Pin new subchannel"
                      @click.stop="
                        () => {
              ...
              <div v-for="subchannel in channel.subchannels">
                <div class="channel-item">
                  <div
                    :class="{
                      'channel-name': true,
                      'selected-channel': selectedChannel == subchannel,
                    }"
                    @click.stop="selectChannel(subchannel)"
                  >
                    {{ subchannel.name }}
                    <span class="channel-actions">
                      <v-btn
                      ...
                      <v-btn
                        v-if="isDirectSubscribed(subchannel)"
                        icon="mdi-sync-off"
                        title="Unsubscribe"
                        @click.stop="
                          () => {
                            unsubscribe(subchannel);
                          }
                        "
                        density="compact"
                        variant="text"
                      ...
                      <v-btn
                        icon="mdi-message-plus-outline"
                        title="New message"
                        @click.stop="openNewMessageDialog(subchannel)"
                        density="compact"
                        variant="text"
                      ></v-btn>
                    </span>
      ...
      <v-col cols="9">
        ...
        <div id="message-list" v-if="filteredMessages.length > 0">
          <div
            ...
            class="message"
            :class="{ viewed: m == selectedMessage }"
  ...
  <v-dialog v-model="showNewMessage" v-if="newMessageChannel">
    <v-card :loading="messageInfoBusy">
      <v-card-title>
        New message to {{ newMessageChannel?.subscriptionScope.main }}
        <template v-if="newMessageChannel?.subscriptionScope.sub !== undefined"
          >/ {{ newMessageChannel?.subscriptionScope.sub }}
        </template>
        as
        ...
        <template v-else>
          ...
          <v-btn icon="mdi-pencil" small @click="showSelectUser = true" title="Change User"></v-btn>
        </template>
      </v-card-title>
      <v-card-subtitle>
