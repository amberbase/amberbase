...
export interface UserWithTenantsAndRoles {
  id: string;
  email: string;
  name: string;
  tenants: { [tenant: string]: string[] };
}
...
type MigrationScript = {
  lvl: number;
  sql?: string;
  migrate?: (conn: mariadb.PoolConnection) => Promise<void>;
};
const migrationScripts: MigrationScript[] = [
  ...
  {
    lvl: 5,
    sql: "CREATE TABLE `documents` (`tenant` VARCHAR(255) NOT NULL,`collection` VARCHAR(50) NOT NULL,`id` VARCHAR(36) NOT NULL,`change_number` INT UNSIGNED NOT NULL DEFAULT '0',`change_user` VARCHAR(36) NULL,`change_time` DATETIME NOT NULL,`data` LONGTEXT NULL,`access_tags` VARCHAR(4096) NULL DEFAULT NULL,PRIMARY KEY (`id`),INDEX `tenant_collection` (`tenant`, `collection`),FULLTEXT INDEX `access_tags` (`access_tags`))",
  },
  {
    lvl: 6,
    sql: "CREATE TABLE `syncactions` (`tenant` VARCHAR(255) NOT NULL,`collection` VARCHAR(50) NOT NULL,`id` VARCHAR(36) NOT NULL,`change_number` INT(10) UNSIGNED NOT NULL DEFAULT '0',`change_time` DATETIME NOT NULL,`access_tags` VARCHAR(4096) NULL DEFAULT NULL,`new_access_tags` VARCHAR(4096) NULL DEFAULT NULL,`deleted` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0,INDEX `tenant_collection` (`tenant`, `collection`,  `change_number`) USING BTREE,FULLTEXT INDEX `access_tags` (`access_tags`))",
  },
...
export class AmberRepo {
  ...
  async initDb() {
    ...
    for (const migration of migrationScripts
      .filter((script) => script.lvl > migrationVersion)
  ...
  async getUserTenantsWithRoles(
    ...
    try {
      var result = await conn.query<{ name: string; tenant: string; roles: string }[]>(
        "SELECT `name`, `id` AS `tenant`, GROUP_CONCAT( `roles` SEPARATOR ',') AS roles FROM tenants RIGHT OUTER JOIN (SELECT `user`, `roles`, `tenant` FROM roles WHERE `user` = ?) AS r ON tenants.id = r.tenant OR r.tenant = '*' GROUP BY `name`,`id`",
        [userId],
      );
      var tenantRoles = result.map((row) => {
        return {
          name: row.name,
          id: row.tenant,
  ...
  async getUsersForTenant(tenant: string): Promise<User[]> {
    var conn = await this.pool.getConnection();
    try {
      var result = await conn.query<{ user: string; email: string; name: string }[]>(
        "SELECT DISTINCT user, email, name FROM roles JOIN users ON roles.`user` = users.id WHERE roles.`tenant` = ? OR roles.`tenant` = '*'",
        [tenant],
      );
      return result.map((user) => {
        return { id: user.user, email: user.email, name: user.name };
      });
    } finally {
      conn.end();
    }
      ...
      if (
        ...
        await conn.query(
          ...
          [
            ...
            changeNumber,
            changeTime,
            arraySetToString(oldDoc.access_tags),
            arraySetToString(accessTags),
          ],
        );
      }
   ...
   */
  async streamAllDocuments(
    tenant: string,
    collection: string,
    newerThanChangeNumber: number | undefined,
    withOneOfTheseAccessTags: string[] | undefined,
    withAllOfTheseTags: string[] | undefined,
    rowCallback: (doc: Document) => void,
  ): Promise<void> {
    var conn = await this.pool.getConnection();
   ...
   * @returns A promise that is only resolved when the stream is finished. The promise will be rejected if an error occurs during streaming.
   */
  async streamAllSyncActions(
    tenant: string,
    collection: string,
    newerThanChangeNumber: number,
    withOneOfTheseAccessTags: string[] | undefined,
    rowCallback: (doc: SyncAction) => void,
  ): Promise<void> {
    var conn = await this.pool.getConnection();
    var accessTagFilter: string | undefined = undefined;
    if (withOneOfTheseAccessTags !== undefined) {
