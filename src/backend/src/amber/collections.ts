...
export interface CollectionSettings<T> {
   ...
   */
  accessRights?:
    | { [role: string]: CollectionAccessAction[] }
    | ((user: UserContext, document: T | null, action: CollectionAccessAction) => boolean);
  /**
...
export interface AmberCollection<T> {
  // the API to be used by the server side app
...
export class CollectionsService implements AmberConnectionMessageHandler, AmberCollections, StatsProvider {
  ...
  constructor(
    config: Config,
    repo: AmberRepo,
    collections: Map<string, CollectionSettings<any>>,
    connectionManager: AmberConnectionManager,
  ...
  enableDebugApi(app: express.Express) {
    ...
    app.get(
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        try {
          var accessTags = collectionSettings.accessTagsFromUser
  ...
  getCollection<T>(collection: string): AmberCollection<T> | undefined {
    ...
    return {
      createDocument: (tenant: string, userId: string | undefined, data: T, documentId?: string) =>
        this.createDocument(tenant, collection, collectionSettings, userId, data, documentId),
      deleteDocument: (tenant: string, userId: string | undefined, documentId: string) =>
        this.deleteDocument(tenant, collection, collectionSettings, userId, documentId),
      updateDocument: (
        tenant: string,
        documentId: string,
        userId: string | undefined,
  ...
  async updateDocumentWithCallback(
    ...
    return await this.updateDocumentWithOld(
      tenant,
      collection,
      collectionSettings,
      documentId,
  ...
  private async handleSubscribe(
    connection: ActiveConnection,
    message: SubscribeCollectionMessage,
    collectionSettings: CollectionSettings<any>,
    ...
    var accessTags =
      collectionSettings.accessTagsFromUser && !isAdmin
        ? collectionSettings.accessTagsFromUser({
    ...
    await this.repo.streamAllDocuments(
      connection.tenant,
      message.collection,
      message.start,
      accessTags,
  ...
  private async handleCreate(
    ...
    var result = await this.createDocument(
      connection.tenant,
      message.collection,
      collectionSettings,
      connection.userId,
    ...
    if (document) {
      for (const connection of this.connectionManager.activeConnectionsForTenant(tenant)) {
        if (connection.items.has(collectionItem(collection))) {
          ...
          let accessTagsForUser =
            collectionSettings.accessTagsFromUser && !connection.roles.includes(tenantAdminRole)
              ? collectionSettings.accessTagsFromUser({
                  userId: connection.userId,
                  roles: connection.roles,
                })
  ...
  async handleDelete(
    ...
    var success = await this.deleteDocument(
      connection.tenant,
      message.collection,
      collectionSettings,
      connection.userId,
  ...
  private async handleUpdate(
    ...
    var success = await this.updateDocumentWithOld(
      connection.tenant,
      message.collection,
      collectionSettings,
      message.documentId,
   ...
   * @returns
   */
  private async updateDocumentWithOld(
    tenant: string,
    collection: string,
    collectionSettings: CollectionSettings<any>,
    documentId: string,
