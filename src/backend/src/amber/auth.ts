...
import {
  ...
  nu,
  error,
  SessionToken as SessionTokenDto,
  RegisterRequest,
  AcceptInvitationRequest,
  UserDetails,
  CreateInvitationRequest,
  UserWithRoles as UserWithRolesDto,
  TenantWithRoles,
  InvitationDetails,
  UserInfo,
...
export async function auth(
  ...
  app.post("/tenant/:tenant/admin/invitation", async (req, res) => {
    ...
    var invitation = await repo.storeInvitation(
      req.params.tenant,
      request.roles,
      new Date(Date.now() + 24 * 60 * 60_000 * request.expiresInDays),
    );
...
export class AmberAuthService implements AmberAuth {
  ...
  async validatePasswordResetToken(resetToken: string): Promise<UserWithCredential | null> {
    ...
    if (new Date(t.expires) < new Date()) return null;
    return user;
  }
